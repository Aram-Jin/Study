...
if __name__ == "__main__":
    ...
    each_user_chat_init = config_json["each_user_chat_init"]
    ...
    #unread_msg_list = s.get(config_json["unread_list_url"].format(channel_id)).json()
    whole_chat_list = s.get(config_json["whole_chat_opened_list"].format(channel_id)).json() # 1안 없어짐
    # dict_keys(['messages', 'sessions', 'userChats', 'users', 'managers', 'chatTags'])
    message = whole_chat_list["messages"]
    userchats = whole_chat_list["userChats"]
    for userchat in userchats:
        if "name" not in userchat:
            continue 
        user_msg = s.get(each_user_chat_init.format(channel_id, userchat['id'])).json()
        print(user_msg)
